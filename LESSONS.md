...
- [2026-09-07] [Pagination/Search] Tìm kiếm bằng keyword gọi `searchByName(keyword)` trả về toàn bộ danh sách không có phân trang dẫn đến hiển thị toàn bộ sản phẩm tìm được lên 1 trang -> Bổ sung `searchByName(keyword, page, pageSize)` và `countSearch(keyword)` trong DAO/Service để phân trang đồng bộ đúng `pageSize = 6`.
